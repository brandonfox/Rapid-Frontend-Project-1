<template>
  <v-card class="py-4 px-6" width="40%" style="min-width: 600px">
    ...
    <v-list v-if="todos.length > 0" class="pr-4">
      <TodoItem v-for="todo in todos" v-bind:key="todo.key" :todo="todo"></TodoItem>
    </v-list>
    <v-card-text v-else>
  ...
  computed: {
    allCompleted: {
      ...
      set: function (newValue) {
        this.todos.forEach(t => this.setCompleted(t,newValue))
      }
    }
  },
  data () {
  ...
  methods: {
    ...
    clearCompleted() {
      this.todos.forEach(t => {
        if(t.val().completed){
          this.$database.ref(this.firebasePath).child(t.key).remove()
        }
      })
    }
  }
